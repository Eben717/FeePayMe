...
const AuditFeeTable: React.FC<Props> = ({ onProcessPayment }) => {
    const { fees, addFee, removeFee, updateFee, clearAllFees, importFees } = useAuditFees();
    ...
    const handleClearAll = () => {
        ...
        if (window.confirm('Are you sure you want to permanently delete ALL audit fee records? This action cannot be undone.')) {
            clearAllFees();
        }
    };
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', padding: '0 0.5rem' }}>
                <input
                    type="file"
                    accept=".xlsx, .xls"
                ...
                <button
                    ...
                    className="btn btn-outline btn-sm"
                    style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', padding: '0.5rem 1rem', border: '1px solid var(--danger, #ef4444)', color: 'var(--danger, #ef4444)', borderRadius: '0.375rem', cursor: fees.length === 0 ? 'not-allowed' : 'pointer', background: 'transparent', fontWeight: 500, opacity: fees.length === 0 ? 0.5 : 1 }}
                    onMouseOver={(e) => { if (fees.length > 0) e.currentTarget.style.backgroundColor = 'rgba(239, 68, 68, 0.1)'; }}
                    onMouseOut={(e) => { if (fees.length > 0) e.currentTarget.style.backgroundColor = 'transparent'; }}
                >
                    <Trash2 size={16} />
                    Clear All Data
                </button>
